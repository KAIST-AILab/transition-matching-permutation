...
def evaluate_with_ground_truth_graph(env, agent, num_games):
    ...
    print_strings.append("EVAL: rewards: {:2.3f} | normalized reward: {:2.3f} | used steps: {:2.3f} | seen recipe: {:2.3f}".format(np.mean(achieved_game_points), np.mean(normalized_game_points), np.mean(total_game_steps), np.mean(total_seen_recipe)))
    for i in range(len(game_name_list)):
        print_strings.append("game name: {}, reward: {:2.3f}, normalized reward: {:2.3f}, steps: {:2.3f}, seen recipe: {}".format(game_name_list[i], achieved_game_points[i], normalized_game_points[i], total_game_steps[i], total_seen_recipe[i]))
    print_strings.append("======================================================")
    print_strings = "\n".join(print_strings)
    print(print_strings)
    return np.mean(achieved_game_points), np.mean(normalized_game_points), np.mean(total_game_steps), 0.0, np.mean(total_seen_recipe), print_strings
...
def evaluate(env, agent, num_games):
    ...
    print_strings = []
    print_strings.append("======================================================")
    print_strings.append(game_name_list[0] + ":    " + " | ".join(print_actions))
    print_strings.append("EVAL: rewards: {:2.3f} | normalized reward: {:2.3f} | used steps: {:2.3f} | seen recipe: {:2.3f}".format(np.mean(achieved_game_points), np.mean(normalized_game_points), np.mean(total_game_steps), np.mean(total_seen_recipe)))
    for i in range(len(game_name_list)):
        print_strings.append("game name: {}, reward: {:2.3f}, normalized reward: {:2.3f}, steps: {:2.3f}, seen recipe: {}".format(game_name_list[i], achieved_game_points[i], normalized_game_points[i], total_game_steps[i], total_seen_recipe[i]))
    print_strings.append("======================================================")
...
def evaluate_rl_with_real_graphs(env, agent, num_games):
    ...
    while(True):
        ...
        chosen_actions_before_parsing = chosen_actions
        recipes = [''] * batch_size
        prev_h, prev_c = None, None
        ########
        ## remove for obs_gen
        ...
        for step_no in range(agent.eval_max_nb_steps_per_episode):
            ...
            observation_strings_for_graph = observation_strings
            if agent.config['rl'].get('recipe_observable'):
                observation_strings = [o + ('' if r in o else bar + r) for o, r in zip(observation_strings, recipes)]

            new_adjacency_matrix, new_graph_hidden_state = agent.generate_adjacency_matrix_for_rl(observation_strings_for_graph, chosen_actions, prev_graph_hidden_state)
    ...
    print_strings.append(game_name_list[0] + ":    " +
                         " | ".join(print_actions))
    print_strings.append(
        "EVAL: rewards: {:2.3f} | normalized reward: {:2.3f} | used steps: {:2.3f} | seen recipe: {:2.3f}"
        .format(np.mean(achieved_game_points), np.mean(normalized_game_points),
                np.mean(total_game_steps), np.mean(total_seen_recipe)))
    for i in range(len(game_name_list)):
        print_strings.append(
            "game name: {}, reward: {:2.3f}, normalized reward: {:2.3f}, steps: {:2.3f}, seen recipe: {}"
            .format(game_name_list[i], achieved_game_points[i],
                    normalized_game_points[i], total_game_steps[i], total_seen_recipe[i]))
    print_strings.append(
        "======================================================")
    print_strings = "\n".join(print_strings)
    print(print_strings)
    return np.mean(achieved_game_points), np.mean(
        normalized_game_points), np.mean(total_game_steps), np.mean(total_seen_recipe), print_strings
...
def evaluate_recipe_reconstruction_free_generation(env, agent, valid_test="valid", verbose=False):
    env.split_reset(valid_test)
    agent.eval()
    total_f1 = []
    counter = 0
    ...
    while(True):
        input_strings, observation_strings, prev_action_strings, recipe_strings = env.get_batch()
        target_strings = recipe_strings
        batch_size = len(observation_strings)
        lens = [len(elem) for elem in observation_strings]
        max_len = max(lens)
        padded_input_strings = [elem + ["<pad>"]*(max_len - len(elem)) for elem in input_strings]
        padded_observation_strings = [elem + ["<pad>"]*(max_len - len(elem)) for elem in observation_strings]
        padded_prev_action_strings = [elem + ["<pad>"]*(max_len - len(elem)) for elem in prev_action_strings]
        padded_recipe_strings = [elem + ["<pad>"]*(max_len - len(elem)) for elem in recipe_strings]
        eps_masks = torch.zeros((batch_size, max_len), dtype=torch.float).cuda() if agent.use_cuda else torch.zeros((batch_size, max_len), dtype=torch.float)
        for i in range(batch_size):
            eps_masks[i, :lens[i]] = 1
        
        if agent.enable_graph_input:
            graph_hidden_state = torch.zeros(batch_size, agent.online_net.block_hidden_dim)
            if agent.use_cuda:
                graph_hidden_state = graph_hidden_state.cuda()
        for j in range(max_len):
            batch_input_string = [elem[j] for elem in padded_input_strings]
            batch_obs_string = [elem[j] for elem in padded_observation_strings]
            batch_prev_act_string = [elem[j] for elem in padded_prev_action_strings]
            batch_recipe_string = [elem[j] for elem in padded_recipe_strings]
        
            if agent.enable_graph_input:
                adjacency_matrix, graph_hidden_state = agent.generate_adjacency_matrix_for_rl(batch_obs_string, batch_prev_act_string, graph_hidden_state)
            else:
                adjacency_matrix = None
            pred_strings = agent.recipe_reconstruction_greedy_generation(batch_input_string, adjacency_matrix)

            for i in range(len(observation_strings)):
                if eps_masks[i, j] == 0:
                    continue # if masked eps then don't compute F1
                _, _, f1 = get_match_result_obs_gen(pred_strings[i], batch_recipe_string[i])
                total_f1.append(f1)
                if verbose:
                    to_print.append(str(counter) + " -------------------------------------------- soft f1: " + str(f1))
                    to_print.append("OBS: %s " % (input_strings[i][j]))
                    to_print.append("PRED: %s " % (pred_strings[i]))
                    to_print.append("GT: %s " % (target_strings[i][j]))
                    to_print.append("")
                counter += 1
        if env.batch_pointer == 0:
            break
    if verbose:
        with open("checkpoints/" + agent.experiment_tag + "/output.txt", "w") as f:
            f.write("\n".join(to_print))
    return np.mean(np.array(total_f1))


def evaluate_recipe_reconstruction_loss(env, agent, valid_test="valid"):
    env.split_reset(valid_test)
    agent.eval()
    ave_loss = []
        ...
        for j in range(max_len):
            ...
            with torch.no_grad():
                if agent.enable_graph_input:
                    ...
                    adjacency_matrix = None
                loss, _ = agent.recipe_reconstruction_teacher_force(batch_input_string, batch_recipe_string, graph_input=adjacency_matrix, episodes_masks=eps_masks[:, j])
            ave_loss.append(to_np(loss))
        if env.batch_pointer == 0:
            break
    return np.mean(np.array(ave_loss))
