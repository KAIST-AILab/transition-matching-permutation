...
class Transition(NamedTuple):
    observation_list: List[Token]
    prev_action_list: List[Token]
    action_candidate_list: List[List[Token]]
    chosen_indices: int
    reward: float
    is_final: bool
...
class PrioritizedReplayMemory(object):
    ...
    def _get_single_transition(self, idx, n):
        ...
        reward = np.sum(np.stack(rewards_up_to_next_final))

        return (obs, prev_action, candidate, chosen_indices, None, reward,
                next_obs, next_prev_action, next_candidate, None)

    def _encode_sample(self, idxes, ns):
        actual_indices, actual_ns = [], []
        obs, prev_action, candidate, chosen_indices, reward, next_obs, next_prev_action, next_candidate = [], [], [], [], [], [], [], []
        ...
        reward = np.stack(reward, 0)  # batch
        actual_ns = np.array(actual_ns)

        return [
            obs, prev_action, candidate, chosen_indices, None, reward,
            next_obs, next_prev_action, next_candidate, None, actual_indices,
            actual_ns
        ]
    ...
    def _get_single_sequence_transition(self, idx, sample_history_length):
        ...
        head = idx
        is_initial = head == 0 or self._storage[head - 1].is_final
        ...
        for m in range(sample_history_length):
            ...
            res.append([
                obs, candidate, chosen_indices, None, reward, next_obs,
                next_candidate, None
            ])
        return res, is_initial
    ...
    def _encode_sample_sequence(self, idxes, sample_history_length):
        ...
        actual_indices = []
        are_initials = []
        # obs, candidate, chosen_indices, _, reward, next_obs, next_candidate, _
        for i in idxes:
            _t = self._get_single_sequence_transition(i, sample_history_length)
            if _t is None:
                continue
            t, is_initial = _t
            actual_indices.append(i)
            are_initials.append(is_initial)
        ...
        for i in range(sample_history_length):
            ...
            res[i][4] = np.stack(res[i][4], 0)  # batch

        return res + [actual_indices, are_initials]
    ...
    def avg_rewards(self):
        ...
        return np.mean(np.stack(rewards))
