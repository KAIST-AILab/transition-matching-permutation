...
def get_scheduler(optimizer, warmup_steps=5000, total_steps=0, decay='none'):
    def lr(steps):
        ...
        if decay == 'linear' and total_steps > warmup_steps:
            ...
            ratio2 = 1.0
        return max(0, min(ratio1, ratio2))
...
class Agent:
    def __init__(self, config):
        ...
        learning_rate_decay = self.config['general']['training']['optimizer'].get('learning_rate_decay', 'none')
        if learning_rate_decay == 'none':
            self.scheduler = None
        else:
            self.scheduler = get_scheduler(self.optimizer,
                self.learning_rate_warmup_until / self.batch_size,
                self.max_episode / self.batch_size,
    ...
    def observation_generation_teacher_force(self, observation_strings, prev_action_strings, episodes_masks, h_t_minus_one, target_strings=None):
    ...
    def recipe_reconstruction_teacher_force(self, observation_strings, target_strings, graph_input, episodes_masks):
        input_target_strings = ["<bos> " + item for item in target_strings]
        output_target_strings = [item + " <eos>" for item in target_strings]
        input_target = self.get_word_input(input_target_strings)
        ground_truth = self.get_word_input(output_target_strings)  # batch x target_length
        ...
        batch_loss = NegativeLogLoss(pred, ground_truth, target_mask, smoothing_eps=self.smoothing_eps)
        loss = torch.sum(batch_loss * episodes_masks) / torch.sum(episodes_masks) # only place where using `episodes_masks`
    ...
    def recipe_reconstruction_greedy_generation(self, observation_strings, graph_input):
        with torch.no_grad():
            batch_size = len(observation_strings)
            # encode
            input_obs = self.get_word_input(observation_strings)
            ...
            for _ in range(self.max_target_length):
                input_target = self.get_word_input([" ".join(item) for item in input_target_token_list])
                pred = self.online_net.decode(input_target, h_og, obs_mask, h_go, node_mask, input_obs)  # batch x time x vocab
                # pointer softmax
                pred = to_np(pred[:, -1])  # batch x vocab
                pred = np.argmax(pred, -1)  # batch
                for b in range(batch_size):
    ...
    def get_game_info_at_certain_step_fully_observable(self, obs, infos):
        ...
        """
        batch_size = len(obs)
        observation_strings = [preproc(item) for item in obs]
        action_candidate_list = []
        for b in range(batch_size):
            ac = [preproc(item) for item in infos["admissible_commands"][b]]
    ...
    def encode(self, observation_strings, graph_input, use_model):
        assert self.task == "rl" or self.task == "recipe_recon"
        model = self.choose_model(use_model)
    ...
    def get_dqn_loss_with_real_graphs(self, episode_no):
        ...
        obs_list, prev_action_list, candidate_list, action_indices, prev_graph_things, rewards, next_obs_list, next_prev_action_list, next_candidate_list, next_prev_graph_things, actual_indices, actual_ns, prior_weights = data

        obs_list_for_graph, prev_graph_hidden_state = zip(*prev_graph_things)
        next_obs_list_for_graph, next_prev_graph_hidden_state = zip(*next_prev_graph_things)

        prev_graph_hidden_state = to_pt(np.stack(prev_graph_hidden_state, 0), enable_cuda=self.use_cuda, type='float')
        new_adjacency_matrix, _ = self.generate_adjacency_matrix_for_rl(obs_list_for_graph, prev_action_list, prev_graph_hidden_state)
        next_prev_graph_hidden_state = to_pt(np.stack(next_prev_graph_hidden_state, 0), enable_cuda=self.use_cuda, type='float')
        next_new_adjacency_matrix, _ = self.generate_adjacency_matrix_for_rl(next_obs_list_for_graph, next_prev_action_list, next_prev_graph_hidden_state)
