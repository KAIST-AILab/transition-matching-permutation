...
class KG_Manipulation(torch.nn.Module):
    ...
    def decode(self, input_target_word_ids, h_og, obs_mask, h_go, node_mask, input_obs):
        ...
        node_mask_square = None if node_mask is None else torch.bmm(trg_mask.unsqueeze(-1), node_mask.unsqueeze(1)) # batch x target_len x node_len
    ...
    def _masked_mean(self, representation, mask):
        _mask = torch.sum(mask, -1)  # batch
        representation = torch.sum(representation, -2)  # batch x hid
        tmp = torch.eq(_mask, 0).float()
        if representation.is_cuda:
            tmp = tmp.cuda()
        _mask = _mask + tmp
        representation = representation / _mask.unsqueeze(-1)  # batch x hid
    ...
    def score_actions(self, input_candidate_word_ids, h_og=None, obs_mask=None, h_go=None, node_mask=None, previous_h=None, previous_c=None):
        ...
        candidate_representations = self._masked_mean(cand_encoding_sequence, cand_mask)  # batch x num_candidate x hid
        ...
        if h_go is not None:
            ...
            graph_representations = self._masked_mean(graph_representations, node_mask)  # batch x hid
        ...
        if h_og is not None:
            ...
            obs_representations = self._masked_mean(obs_representations, obs_mask) # batch x hid
