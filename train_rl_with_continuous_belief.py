...
def train():
    ...
    running_avg_seen_recipe = HistoryScoreCache(capacity=500)
    ...
    while episode_no <= agent.max_episode:
        ...
        for step_no in range(agent.max_nb_steps_per_episode):
            ...
            if config['rl'].get('recipe_observable'):
                observation_strings = [o + r for o, r in zip(observation_strings, recipes)]
            ...
            replay_info = [observation_strings, action_candidate_list, chosen_indices, observation_strings_for_graph, generic.to_np(prev_graph_hidden_state), chosen_actions]
        ...
        for b in range(game_rewards_np.shape[1]):
            ...
            for i in range(game_rewards_np.shape[0]):
                observation_strings, action_candidate_list, chosen_indices, observation_strings_for_graph, graph_hidden_state, prev_action_strings = transition_cache[i]
                is_final = True
                if still_running_mask_np[i][b] != 0:
                    is_final = False
                agent.dqn_memory.add(observation_strings[b], prev_action_strings[b], action_candidate_list[b], chosen_indices[b], (observation_strings_for_graph[b], graph_hidden_state[b]), command_rewards_pt[i][b], graph_rewards_pt[i][b], count_rewards_pt[i][b], is_final)
                if still_running_mask_np[i][b] == 0:
                    break
            if _need_pad:
                observation_strings, action_candidate_list, chosen_indices, observation_strings_for_graph, graph_hidden_state, prev_action_strings = transition_cache[-1]
                agent.dqn_memory.add(observation_strings[b], prev_action_strings[b], action_candidate_list[b], chosen_indices[b], (observation_strings_for_graph[b], graph_hidden_state[b]), command_rewards_pt[-1][b] * 0.0, graph_rewards_pt[-1][b] * 0.0, count_rewards_pt[-1][b] * 0.0, True)
        ...
        for b in range(batch_size):
            ...
            running_avg_seen_recipe.push(seen_recipe[b])
        ...
        _s = json.dumps({"time spent": str(time_2 - time_1).rsplit(".")[0],
                         ...
                         "eval seen recipe": str(eval_seen_recipe),
